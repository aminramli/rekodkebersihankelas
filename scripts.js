        ...
        fetch(scriptUrl, { method: "GET" })
            ...
            .then(data => {
                if (!Array.isArray(data)) {
                    document.getElementById("recordStatus").innerHTML = "<p>Tiada data tersedia dalam Google Sheet</p>";
                    return;
                }
                ...
                months.forEach(month => {
                    const recorded = data.filter(row => {
                        const bulan = (row.bulan || "").toUpperCase();
                        return bulan === month;
                    }).map(row => `${row.tingkatan}-${row.kelas || "N/A"}`);
                    ...
                    recordStatus.innerHTML += `
                        ...
                        <ul>
                            ${recorded.length > 0 
                                ...
                                : "<li style='color: gray'>Tiada rekod kelas di sini</li>"}
                        </ul>
                    `;
                });

                if (!hasData) {
                    recordStatus.innerHTML = "<p>Tiada rekod ditemui untuk mana-mana bulan</p>";
                }
            })
            .catch(error => {
